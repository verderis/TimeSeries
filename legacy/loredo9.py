...
for shape in shape_list:
    ...
    for pixel in pixelsInside:
        samplePlots[str(ID)+'_'+str(pixelNumber)] = pixel
        pixelNumber += 1
    ID += 1
        
#    samplePlots[name] = evi[inside]
    
#    plt.scatter(lon[inside], lat[inside])    
#    f.plot_shape(shape)

#plt.show()

pixel = samplePlots.keys()[2] #analized pixel
sample_data = samplePlots[pixel]
...
for new in rango:
    ...
    #%% breackpoint detection
    nonZero = y.nonzero() #nonzero datarange selection
#    whereUp = sample_data[23:] > y_095[nonZero][23:]
#    whereDown = sample_data[23:] < y_005[nonZero][23:]
    whereUp = sample_data > y_095[nonZero]
    whereDown = sample_data < y_005[nonZero]
    
    if whereUp.any():
#        for brk in whereUp:
#            if brk:
        breakPointsUp.append(t[whereUp])
#        break
        
    if whereDown.any():
        breakPointsDown.append(t[whereDown])
        break
    ...
    plt.plot(fechas[nonZero], y[nonZero], alpha=0.7, 
             linestyle = ':', color='blue', label='mean')
    plt.fill_between(fechas[nonZero], y_005[nonZero], 
                     y_095[nonZero], alpha=0.3, color=fillColor,
                     label='CI')
    # real data plotting
